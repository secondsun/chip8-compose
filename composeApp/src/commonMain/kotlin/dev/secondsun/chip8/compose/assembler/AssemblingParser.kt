...
data class IntExpression(val expression: List<Token>) {
    ...
    private fun parseCalc(inExpression: MutableList<Token>) : Double {
        val first = inExpression.removeFirst()
        if (first is Token.Identifier) {
            return when (first.name) {
                "strlen" -> (inExpression.removeFirst() as Token.StringToken).value.length.toDouble()
                "sin" -> { sin(parseCalc(inExpression)) }
                "cos" -> { cos(parseCalc(inExpression)) }
                "tan" -> { tan(parseCalc(inExpression)) }
                "exp" -> { exp(parseCalc(inExpression)) }
                "log" -> {
                    ln(parseCalc(inExpression))
                }
                "abs" -> { abs(parseCalc(inExpression)) }
                "sqrt" -> { sqrt(parseCalc(inExpression)) }
                "sign" -> { sign(parseCalc(inExpression)) }
                "ceil" -> { ceil(parseCalc(inExpression)) }
                "floor" -> { floor(parseCalc(inExpression)) }
                "pow" -> { exp(parseCalc(inExpression)) }
                "min" -> {
                    min(parseTerminal(inExpression), parseCalc(inExpression))
                }
                "max" -> {max(parseTerminal(inExpression), parseCalc(inExpression))}
    ...
    private fun parseExpression(inExpression: MutableList<Token>):Int {
        ...
        val lhs = inExpression.removeFirst()
