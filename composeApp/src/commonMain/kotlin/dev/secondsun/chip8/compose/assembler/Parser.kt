...
class ParserContext(program: List<Token>, source:String) : ParserOutput {
    var hasErrors = false;
    var dict = mutableMapOf<String,Int>()//LabelToAddress
    var resolvedAliases = mutableMapOf<String, Int>()//Registers
    ...
    private fun inst(a: Int, b: Int, line: Int) {
        this.data(a, line)
        this.data(b, line)

    }

    fun data(a: Int, line: Int) {
        ...
        this.dbginfo.mapAddr(this.hereaddr, line)
        this.hereaddr++
    }


    fun handleOrg(): ParsedToken {
    ...
    fun defineStringMode(): ParsedToken {
    ...
    fun consumeMacroBody(): List<Token> {
    ...
    fun consumeMacroDefArguments(): List<Token> {
    ...
    fun defineBreakpoint(): ParsedToken {
    ...
    fun defineConstant(): ParsedToken {
    ...
    fun calculateAlias(): Pair<List<Token>, IntExpression> {
        when (val next = tokenProvider.peek()) {
            is Token.Number -> {
                tokenProvider.consume<Token.Number>()
    ...
    fun calculateConstant(): Pair<List<Token>, IntExpression> {
        when (val next = tokenProvider.peek()) {
            is Token.Number -> {
                tokenProvider.consume<Token.Number>()
                return Pair(listOf(next), IntExpression(listOf(next)))
    ...
    fun defined(label: String): Boolean {
    ...
    fun parseOne(): ParsedToken {
        return when (val token = tokenProvider.peek()) {
            ...
            is Token.Number -> {
                consumeNumber().ifCompiles{data(token.value, token.line)}
            }
    ...
    fun consumeStringMode(): ParsedToken {
    ...
    fun consumeIAssign(): ParsedToken {
    ...
    fun consumeSaveFlags(): ParsedToken {
    ...
    fun consumeLoadFlags(): ParsedToken {
        val saveFlags = tokenProvider.consume<Token.SaveFlags>()
        val register = tokenProvider.consume<Any>()
    ...
    fun isRegister(register: Token): Boolean {
    ...
    fun consumeScrollDown(): ParsedToken {
    ...
    fun consumeScrollUp(): ParsedToken {
        val plane = tokenProvider.consume<Token.Plane>()
        val planeNumber = tokenProvider.consume<Any>()
    ...
    fun consumeNative(): ParsedToken {
        return consumeOperatorWithWideParameter(ParsedTokenType.Native)
    }

    fun consumeOperatorWithWideParameter(jumpType: ParsedTokenType): ParsedToken {
    ...
    fun consumeCondition(): ParsedToken {
    ...
    fun handleSave(): ParsedToken {
        val save = tokenProvider.consume<Token.Save>()
        val reg = tokenProvider.peek()
        if (reg is Token.Identifier && isRegister(reg.name)) {
            tokenProvider.consume<Token.Identifier>()
            Registers.valueOf(reg.name.lowercase())
        } else if (reg is Token.Identifier) {
            tokenProvider.consume<Token.Identifier>()
        ...
        if (tokenProvider.hasMore()) {
            ...
            if (next is Token.Minus) {
                val minus = tokenProvider.consume<Token.Minus>()
                val reg2 = tokenProvider.peek()
                if (reg2 is Token.Identifier && isRegister(reg2.name)) {
                    tokenProvider.consume<Token.Identifier>()
                    Registers.valueOf(reg2.name.lowercase())
                } else if (reg2 is Token.Identifier) {
                    tokenProvider.consume<Token.Identifier>()
    ...
    fun handleAssert(): ParsedToken {
        ...
        return if (message is Token.StringToken) {
            (ParsedToken(ParsedTokenType.Assert, listOf(assert) + message + expression))
        } else {
            (ParsedToken(ParsedTokenType.Assert, listOf(assert) + expression))
        }
    ...
    fun parseTokens() {
        while (tokenProvider.hasMore()) {
            ...
            if (token is Token.Proto) {
                ...
                this.hasErrors = (parsed.type == ParsedTokenType.Error || this.hasErrors)
    ...
    private fun ParsedToken.ifCompiles(function: (ParsedToken) -> Unit): ParsedToken {

        if (!this@ParserContext.hasErrors && this.type != ParsedTokenType.Error) {
            function(this)
        }
        return this
    }
...
fun parse(program: String): ParserOutput {
    ...
    return parse(tokens, program)
}

fun parse(program: List<Token>, source:String): ParserOutput {
    val context = ParserContext(program, source)
    context.setup()
    context.parseTokens()
    return context
